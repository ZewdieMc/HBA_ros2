...
int main(int argc, char **argv)
{
    ...
    auto node = rclcpp::Node::make_shared("hba");

    int total_layer_num, thread_num;
    string data_path;

    node->declare_parameter<int>("total_layer_num", 0);
    node->declare_parameter<int>("thread_num", 1);
    node->declare_parameter<int>("pcd_name_fill_num", 0);
    node->declare_parameter<std::string>("data_path", "");

    node->get_parameter("total_layer_num", total_layer_num);
    node->get_parameter("pcd_name_fill_num", pcd_name_fill_num);
    node->get_parameter("data_path", data_path);
    node->get_parameter("thread_num", thread_num);
